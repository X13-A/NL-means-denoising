...
int apply_nlmeans_ipol(const char* d_v_arg, const char* fSigma_arg, const char* add_noise_arg, const char* fname_arg, const char* fname2_arg)
{
    
    // read input
    size_t nx,ny,nc;
    float *d_v = NULL;
    d_v = io_png_read_f32(d_v_arg, &nx, &ny, &nc);
    if (!d_v) {
    ...
    // add noise
    float fSigma = atof(fSigma_arg);
    float *noisy = new float[d_whc];
    int add_noise = atoi(add_noise_arg);	
	...
	if (add_noise == 1) {
		...
		if (io_png_write_f32(fname2_arg, noisy, (size_t) d_w, (size_t) d_h, (size_t) d_c) != 0) {
			printf("... failed to save png image %s", fname2_arg); }
		}	

			
	if (io_png_write_f32(fname_arg, denoised, (size_t) d_w, (size_t) d_h, (size_t) d_c) != 0) {
        printf("... failed to save png image %s", fname_arg); }
...
int main(int argc, char **argv) 
{
    ...
    apply_nlmeans_ipol(argv[1], argv[2], argv[3], argv[4], argv[5]);
