...
int apply_img_diff_ipol(const char* d_v_arg, const char* d_v2_arg, const char* fSigma_arg, const char* fname_arg)
{
    // read input1
    size_t nx,ny,nc;
    float *d_v = NULL;
    ...
    size_t nx2,ny2,nc2;
    float *d_v2 = NULL;
    d_v2 = io_png_read_f32(d_v2_arg, &nx2, &ny2, &nc2);
    if (!d_v2) {
        printf("error :: %s not found  or not a correct png image \n", d_v2_arg);
        return EXIT_FAILURE;
    }
    ...
    if (io_png_write_f32(fname_arg, difference, (size_t) d_w, (size_t) d_h, (size_t) d_c) != 0) {
...
int main(int argc, char **argv) 
{
    ...
    apply_img_diff_ipol(argv[1], argv[2], argv[3], argv[4]);
